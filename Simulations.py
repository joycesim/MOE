...
###############################################
# 
n_oceans= 0.4 #=0.04 #0.8#1.0#0.8
# n_oceans=np.array([0.6,0.7,0.8,1.0])
...
redox_fac=0.01
# redox_fac= np.array([0.1,1.0,10.0,100.0,1000.0]) # 0.1,1.0,10.0,100.0,1000.0
# redox_fac= np.array([0.01,0.1,1.0,10.0,100.0,1000.0,1.0e4])

      
# [1.0,0.5,0.3,0.2,0.1,0.01,0.001]
...
#     mars=Mars(noceans=n_oceans[ii],HoverC=H_over_C, nsteps=20000,redox_factor=redox_fac)
...
# for ii in range (0,7):
# # for ii in range (0,2):
#     mars=Mars(noceans=n_oceans,HoverC=H_over_C, nsteps=50000,redox_factor=redox_fac[ii],  tau = Tautest) #, )
#     mars.time_marching(4.0,const_Ftl=False)
#     # mars.time_marching(4.0,const_Ftl=True)
#     # mars.output_filenames()
#     mars.create_output()
...
mars=Mars(noceans=n_oceans,HoverC=H_over_C, nsteps=20000,redox_factor=redox_fac)  #, tau = Tautest) #, Fcl=True)
